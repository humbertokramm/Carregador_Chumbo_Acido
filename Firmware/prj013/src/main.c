  ...
  __GPIOA_CLK_ENABLE();
    ...
    if (HAL_ADCEx_Calibration_Start(&AdcHandle) != HAL_OK)
   {
	...
	if (HAL_ADC_PollForConversion(&AdcHandle, 1000000) == HAL_OK)
	{
		...
		val = (float)((g_ADCValue1/* - 3103 */) * (3.3/4096) *5);

		Buffer[0] = Buffer2[0] = 0;

		//val = 1;
		 itoa((int)val, (uint8_t*)Buffer, 10);
		 strcat(Buffer,".");
		 itoa((val - (int)val)*10, (uint8_t*)Buffer2, 10);
		 strcat(Buffer, Buffer2);
		 strcat(Buffer," Amp");

		 HAL_UART_Transmit(&UartHandle, "\r Corrente-> ", 13, 100);
		 HAL_UART_Transmit(&UartHandle, (uint8_t*)Buffer, 8, 100);
	...
	// Tensão da Bateria, Razão do divisor de tensão 3,93
		...
		val = (float)(g_ADCValue2 * (3.3/4096) * 6.8);
		 ...
		 HAL_UART_Transmit(&UartHandle, "\r Tensao Bateria -> ", 20, 100);
	...
	if (HAL_ADC_PollForConversion(&AdcHandle, 1000000) == HAL_OK)
	{
		...
		// Temperatura 0 mV + 10.0 mV/°C
		 ...
		 val = (float)(g_ADCValue3 * ((3.3/4096)*100));

		 itoa((int)val, (uint8_t*)Buffer, 10);
		 strcat(Buffer,".");
		 itoa((val - (int)val)*10, (uint8_t*)Buffer2, 10);
		 strcat(Buffer, Buffer2);
		 strcat(Buffer," Celsius");

		 HAL_UART_Transmit(&UartHandle, "\r Temperatura -> ", 18, 100);
		 HAL_UART_Transmit(&UartHandle, (uint8_t*)Buffer, 12, 100);
		 HAL_UART_Transmit(&UartHandle, "\n\t\n\t", 4, 100);
		 HAL_Delay(10);
	}
